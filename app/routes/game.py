...
def flappy_save_progress():
    ...
    def unlock_skin(code, name, icon_emoji):
        ...
        if code not in unlocked:
            ...
            crea_notifica('skin_unlock', f'🦅 {player_name} ha sbloccato la skin {name}!', icon=icon_emoji, send_push=True)
    ...
    # Check High Score & Top 3 General (Same as save_score but integrated to save_progress if needed)
    ...
    if score > old_high_score:
        ...
        for idx, player in enumerate(top_players):
            if player.id == current_user.id:
                ...
                crea_notifica('flappy_leaderboard', f'{emoji} {get_nome_giocatore(current_user)} ha raggiunto il {position_text} posto in Floppy Eagle con {score} punti!', icon=emoji)
                break
...
def save_score():
    ...
    new_record = score > current_user.flappy_high_score
    ...
    if monthly_score:
        if score > monthly_score.score:
            ...
            monthly_score.date_achieved = now
            monthly_improved = True
    else:
        monthly_score = FlappyMonthlyScore(user_id=current_user.id, score=score, month=now.month, year=now.year)
    ...
    if monthly_improved:
        top3_mensile = FlappyMonthlyScore.query.filter(
            FlappyMonthlyScore.month == now.month, FlappyMonthlyScore.year == now.year
        ).order_by(FlappyMonthlyScore.score.desc()).limit(3).all()
        
        for i, ms in enumerate(top3_mensile):
...
def flappy_complete_mission():
    ...
    def check_mission_unlock(req, code, name, icon):
        ...
        if profile.missions_completed_count >= req and code not in unlocked:
            ...
            crea_notifica('skin_unlock', f'🦅 {get_nome_giocatore(current_user)} ha sbloccato la skin {name}!', icon=icon, send_push=True)
